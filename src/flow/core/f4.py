...
class Flow:
    ...
    #     if self._parent_flow:
    #         # Get parent's result first
    #         parent_result = await self.context.results_manager.get_result(self._parent_flow.process_id)
    #         if parent_result and parent_result.output:
    #             input_data.update(parent_result.output)
    ...
    #                 )
    #             # Get the result from the results manager
    #             stored_result = await self.context.results_manager.get_result(dep_id)
    #             if stored_result and stored_result.output:
    #                 deps_data.update(stored_result.output)
    #             logger.debug(f"Got dependency result for {dep_flow.config.name}: {stored_result}")
    ...
    #     try:
    #         # Execute dependencies and get their outputs
    #         deps_data = await self._execute_dependencies(input_data)
            
    #         # Merge with input data (input_data takes precedence)
    #         execution_data = {**deps_data, **input_data}
    #         logger.debug(f"Merged execution data for {self.config.name}: {execution_data}")
    ...
    #         # Save result immediately after successful execution
    #         await self.context.results_manager.save_result(
    #             self.process_id,
    #             result
    #         )
    #         logger.debug(f"Saved result for {self.config.name}: {result}")

    #         # Now execute any dependent flows
    #         for dep_id in self._dependent_flows:
    #             dep_flow = self.context.get_flow(dep_id)
    #             if dep_flow:
    #                 logger.debug(f"Executing dependent flow {dep_flow.config.name}")
    #                 await dep_flow.execute(input_data)
    ...
    async def _execute_dependencies(
        ...
        # Get required dependencies' results
        required_deps = self.get_dependencies(DependencyType.REQUIRED)
        if required_deps:
            logger.debug(f"Getting required dependencies for {self.config.name}: {required_deps}")
            for dep_id in required_deps:
                dep_result = await self.context.results_manager.get_result(dep_id)
                if not dep_result or dep_result.status != FlowStatus.COMPLETED:
                    raise FlowError(f"Required dependency {dep_id} not completed")
                deps_data.update(dep_result.output or {})

        # Get optional dependencies' results
        optional_deps = self.get_dependencies(DependencyType.OPTIONAL)
        if optional_deps:
            logger.debug(f"Getting optional dependencies for {self.config.name}: {optional_deps}")
            for dep_id in optional_deps:
                dep_result = await self.context.results_manager.get_result(dep_id)
                if dep_result and dep_result.status == FlowStatus.COMPLETED:
                    deps_data.update(dep_result.output or {})
