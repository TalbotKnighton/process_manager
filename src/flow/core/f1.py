...
from process_manager.workflow.workflow_types import ProcessResult
...
class Flow:
    ...
    async def _execute_with_dependencies(
        ...
        try:
            ...
            # Execute the processor
            process_result = await self._execute_processor(execution_data)
            
            # Update flow result based on process result
            if process_result.is_successful():
                result.status = FlowStatus.COMPLETED
                result.output = process_result.output
                result.metadata = process_result.metadata
            else:
                result.status = FlowStatus.FAILED
    ...
    async def _execute_dependencies(
        ...
        for dep_id, dep_type in self._dependencies.items():
            ...
            try:
                ...
                if dep_type == DependencyType.REQUIRED:
                    raise FlowError(f"Required dependency {dep_id} failed: {str(e)}")
                logger.warning(f"Optional dependency {dep_id} failed: {e}")
    ...
    async def _execute_processor(
        ...
        """Execute the processor with error handling and result standardization."""
        try:
            self.status = FlowStatus.RUNNING
            
            # Execute in process pool if configured
            if self.config.flow_type in (FlowType.PROCESS, FlowType.THREAD):
                if hasattr(self.processor, 'process'):
                    raw_result = await self.context.pool_manager.submit_task(
                        self.process_id,
                        self.config.flow_type,
                        self.processor.process,
                        input_data,
                ...
                # Execute directly if inline
                if hasattr(self.processor, 'process'):
                    raw_result = self.processor.process(input_data)
                else:
                    raw_result = self.processor(input_data)
